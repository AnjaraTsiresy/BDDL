...
class ImpressionController extends Controller
{
    ...
    public function checkimpressionAction($id)
    {
       ...
       // $em = $this->getDoctrine()->getManager();
        $sqlvidage = "DELETE FROM `temp_Pdf_LoadDataTheme` WHERE id='$id'";
        $this->execute($sqlvidage);

        $sqlPopulationTable = "INSERT INTO `temp_Pdf_LoadDataTheme` (
`lib` ,
`idT` ,
`description` ,
`id_societe` ,
...
SELECT distinct(libelle_theme) as lib, theme.id_theme as idT, societe.description as description,
	lexique.id_societe as id_societe , lexique.id_prototype_access as idProt FROM lexique
	INNER JOIN theme ON theme.id_theme = lexique.id_theme
	INNER JOIN societe ON societe.id_societe = lexique.id_societe
	WHERE lexique.id_prototype_access = $id
	AND theme.libelle_theme IS NOT NULL ORDER BY lexique.rang";
        $this->execute("set sql_big_selects=1");

        $this->execute($sqlPopulationTable);
        echo "OK";
